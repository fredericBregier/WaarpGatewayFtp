...
public class FtpConstraintLimitHandler extends WaarpConstraintLimitHandler {
   ...
   *     JavaSysMon
   * @param lowcpuLimit for proactive cpu limitation (throttling bandwidth)
   *     (0<= x < 1 & highcpulimit)
   * @param highcpuLimit for proactive cpu limitation (throttling bandwidth)
   *     (0<= x <= 1) 0 meaning no throttle
   *     activated
   * @param percentageDecrease for proactive cpu limitation, throttling
   *     bandwidth reduction (0 < x < 1) as 0.25 for
   *     25% of reduction
   * @param handler the GlobalTrafficShapingHandler associated (null to have
   *     no proactive cpu limitation)
   * @param delay the delay between 2 tests for proactive cpu limitation
   * @param limitLowBandwidth the minimal bandwidth (read or write) to apply
   *     when decreasing bandwidth (low limit =
   *     4096)
   */
   ...
   */
  public FtpConstraintLimitHandler(long timeoutcon,
                                   boolean useCpuLimit, boolean useJdKCpuLimit,
                                   double cpulimit,
