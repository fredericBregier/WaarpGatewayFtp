...
public class FtpPrivateMib implements WaarpInterfaceMib {
  ...
  protected void agentRegisterWaarpMib()
    ...
    WaarpMOScalar memoryScalar =
        rowGlobal.getRow()[WaarpGlobalValuesIndex.memoryTotal
            .ordinal()];
  ...
  public void changeStatus(OperStatus status) {
    WaarpMOScalar statusScalar =
        rowGlobal.getRow()[WaarpGlobalValuesIndex.applOperStatus
            .ordinal()];
    Integer32 var = (Integer32) statusScalar.getValue();
    if (var.getValue() != status.status) {
      var.setValue(status.status);
      WaarpMOScalar lastTimeScalar =
          rowGlobal.getRow()[WaarpGlobalValuesIndex.applLastChange
              .ordinal()];
