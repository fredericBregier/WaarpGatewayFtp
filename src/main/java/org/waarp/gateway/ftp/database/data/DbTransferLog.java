...
public class DbTransferLog extends AbstractDbData {
  ...
  public static DbPreparedStatement getCountInOutErrorPrepareStatement(
    ...
    if (in) {
      inCond =
          " (" + Columns.MODETRANS.name() + " = '" + FtpCommandCode.APPE.name()
          + "' OR " +
          Columns.MODETRANS.name() + " = '" + FtpCommandCode.STOR.name() +
          "' OR " +
   ...
   * @param purge sets whether or not the selected results must be purged
   *
   * @return The message for the HTTPS interface
   */
  public static String saveDbTransferLogFile(
  ...
  protected void setFromArray() throws WaarpDatabaseSqlException {
    ...
    try {
      infostatus =
          ReplyCode.getReplyCode(((Integer) allFields[Columns.INFOSTATUS
              .ordinal()].getValue()));
    } catch (InvalidArgumentException e) {
