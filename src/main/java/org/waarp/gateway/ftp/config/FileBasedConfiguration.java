...
public class FileBasedConfiguration extends FtpConfiguration {
   ...
   * multiple of 8192 (maximum = 64K due to block
   ...
   * @param purge if True, the current authentications are totally replaced
   *     by the new ones
   *
   * @return True if OK
   */
  ...
  public void configureHttps() {
    ...
    httpExecutor =
        new NioEventLoopGroup(getSERVER_THREAD() * 10, new WaarpThreadFactory(
            "HttpExecutor"));
  ...
  public void configureLExec() {
    if (useLocalExec) {
      LocalExecClient
          .initialize(this.getCLIENT_THREAD(), this.getMaxGlobalMemory());
    }
  }
  ...
  public String getHtmlTransfer(String format, int limit) {
    ...
    try {
      try {
        preparedStatement =
            DbTransferLog
                .getStatusPrepareStament(DbConstant.gatewayAdmin.getSession(),
                                         null, limit);
